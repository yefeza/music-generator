...
def train_epochs(g_model, d_model, gan_model, dataset, n_epochs, n_batch, fadein=False):
	...
	for i in range(n_steps):
		...
		gradient_penalty=get_gradient_penalty(X_fake, X_real, half_batch, d_model)
...
def train(g_models, d_models, gan_models, dataset, latent_dim, e_norm, e_fadein, n_batch, job_dir):
	...
	for i in range(1, len(g_models)):
        ...
        #guardar modelos
        guardar_modelo(g_normal, job_dir, str(gen_shape[-3])+"x"+str(gen_shape[-2]))
...
# train model
train(generators, discriminators, composite, dataset, latent_dim, NUM_EPOCHS, NUM_EPOCHS, BATCH_SIZE, JOB_DIR)
