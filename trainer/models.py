...
def G_wgan_acgan(y_true, y_pred):
    ...
    with tf.name_scope('LabelPenalty'):
        label_penalty_fakes = tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(labels=y_true, logits=fake_scores_out)
...
def D_wgangp_acgan(y_true, y_pred, gradient_penalty):
    ...
    with tf.name_scope('LabelPenalty'):
        label_penalty_reals = tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(labels=y_true_real_images, logits=real_scores_out)
        label_penalty_fakes = tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(labels=y_true_fake_images, logits=fake_scores_out)
...
def add_discriminator_block(old_model,n_input_layers=3):
  ...
  #compilemodel
  model1.compile(loss=None,optimizer=Adam(lr=0.001,beta_1=0,beta_2=0.99,epsilon=10e-8))
  ...
  #compilemodel
  model2.compile(loss=None,optimizer=Adam(lr=0.001,beta_1=0,beta_2=0.99,epsilon=10e-8))
...
def define_discriminator(n_blocks, lstm_layer, input_shape=(4,750,2)):
	...
	# compile model
	model.compile(loss=None, optimizer=Adam(lr=0.001, beta_1=0, beta_2=0.99, epsilon=10e-8))
...
def add_generator_block(old_model):
	...
	g = Conv2D(128, (3,3), padding='same', kernel_initializer='he_normal')(upsampling)
...
def define_composite(discriminators, generators):
	...
	for i in range(len(discriminators)):
		...
		model1.compile(loss=G_wgan_acgan, optimizer=Adam(lr=0.001, beta_1=0, beta_2=0.99, epsilon=10e-8))
		# fade-in model
		d_models[1].trainable = False
		model2 = Sequential()
		model2.add(g_models[1])
		model2.add(d_models[3])
		model2.compile(loss=G_wgan_acgan, optimizer=Adam(lr=0.001, beta_1=0, beta_2=0.99, epsilon=10e-8))
		# store
