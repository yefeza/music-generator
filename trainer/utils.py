...
def generate_fake_samples(g_model, latent_dim, half_batch):
  noise = np.random.uniform(-1, 1, (half_batch, latent_dim[0], latent_dim[1] , latent_dim[2]))
...
def generate_latent_points(latent_dim, n_batch):
  noise = np.random.uniform(-1, 1, (n_batch, latent_dim[0], latent_dim[1] , latent_dim[2]))
  return noise
...
def get_gradient_penalty(fake_images, real_images, minibatch_size, d_model, wgan_target=1.0):
  with tf.name_scope('GradientPenalty'):
    mixing_factors = tf.compat.v1.random_uniform([minibatch_size, 1, 1, 1], 0.0, 1.0)
...
def generar_ejemplos(g_model, prefix, n_examples, job_dir):
  ...
  gen_auds=g_model.predict(noise)
  gen_auds=np.reshape(gen_auds, newshape=(n_examples, (gen_shape[-3]*gen_shape[-2]), 2))
  for i in range(n_examples):
    signal_gen=gen_auds[i]
    signal_gen /= np.max(np.abs(signal_gen),axis=0)
    path_save=job_dir + "/" + str(gen_shape[-3]) + "x" + str(gen_shape[-2]) + "/" + prefix + "-" + str(i) + '.wav'
